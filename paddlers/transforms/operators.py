...
__all__ = [
    ...
    "ArrangeClassifier", "ArrangeDetector", "RandomFlipOrRotation","MatchHistograms"
]
...
class MatchHistograms(Transform):
    """
    Histogram matching for image with image_reference as reference

    """
    def __init__(self):
        super(MatchHistograms, self).__init__()

    def apply(self,sample):
        image0 = sample['image']
        image = image0[:, :, ::-1]  # 转换成RGB
        reference0 = sample['image2']
        image_reference = reference0[:, :, [2, 1, 0]]  # 转换成RGB

        matched = match_histograms(image, image_reference, multichannel=True)  # 直方图匹配
        sample['image']=matched
        return sample